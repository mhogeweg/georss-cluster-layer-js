define([
  ...
  return declare([GraphicsLayer], {
    ...
    add: function(p) {
      ...
      // add the new data to _georssData so that it's included in clusters
      // when the map level changes
      this._georssData.push(p);
      //var clustered = false;
      // look for an existing cluster for the new point
      /*for ( var i = 0; i < this._clusters.length; i++ ) {
        var c = this._clusters[i];
        if ( this._clusterTest(p, c) && false ) {
    ...
    onClick: function(e) {
      ...
      var singles = [];
      for ( var i = 0, il = this._georssData.length; i < il; i++) {
        if ( e.graphic.attributes.clusterId == this._georssData[i].attributes.clusterId ) {
          singles.push(this._georssData[i]);
        }
      }
    ...
    // internal methods 
    _drawGeoRSS: function() {
      // first time through, loop through the points
      for ( var j = 0, jl = this._georssData.length; j < jl; j++ ) {
        // see if the current feature should be added to a cluster
        var point = this._georssData[j];
        var clustered = false;
    ...
    _clusterMeta: function() {
      ...
      console.log("Total:  ", this._georssData.length);
